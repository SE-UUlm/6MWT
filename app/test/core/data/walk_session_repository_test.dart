...
void main() {
  ...
  test('saves results and lists them newest first', () async {
    ...
    expect(results.last.duration, 360); // 6 minutes in seconds
  ...
  test('watchResults emits updates when a new session is saved', () async {
    // Take 2 emissions: initial (empty) and after the insert.
    final future = repository.watchResults().take(2).toList();
  ...
  test('saves zero-distance aborted session correctly', () async {
    ...
    expect(results.single.duration, 5);
